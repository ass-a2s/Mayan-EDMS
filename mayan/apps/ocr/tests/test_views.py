...
from .literals import TEST_DOCUMENT_VERSION_CONTENT
...
class DocumentOCRViewsTestCase(
    ...
    def test_document_content_view_with_access(self):
        ...
        response = self._request_test_document_version_ocr_content_view()
        self.assertContains(
            response=response, text=TEST_DOCUMENT_VERSION_CONTENT, status_code=200
        )
    ...
    def test_document_submit_view_with_access(self):
        ...
        self.assertTrue(
            TEST_DOCUMENT_VERSION_CONTENT in ''.join(

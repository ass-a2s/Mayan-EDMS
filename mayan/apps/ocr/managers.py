...
class DocumentVersionPageOCRContentManager(models.Manager):
    def delete_content_for(self, document_version, user=None):
        with transaction.atomic():
            for document_version_page in document_version.pages.all():
                self.filter(document_version_page=document_version_page).delete()
    ...
    def process_document_version_page(self, cache_filename, document_version_page):
        logger.info(
        ...
        try:
            ...
            logger.error(
                'OCR error for document page: %d; %s', document_version_page.pk,
                exception
            )
            raise
        else:

...
def task_document_version_ocr_process(self, document_version_pk):
    logger.info('Starting OCR for document file page: %s', document_version_pk)
            ...
            page_tasks.append(
                chain(
                    ...
                    task_document_version_page_ocr_process.s(
                        document_version_page_pk=document_version_page.pk
                    )
                )
            )
        chord(page_tasks)(task_document_version_ocr_finished.s(document_version_pk=document_version.pk))
...
@app.task(bind=True, default_retry_delay=DO_OCR_RETRY_DELAY, ignore_result=True)
def task_document_version_page_ocr_process(self, cache_filename, document_version_page_pk):
...
@app.task(bind=True, default_retry_delay=DO_OCR_RETRY_DELAY, ignore_result=True)
def task_document_version_ocr_finished(self, results, document_version_pk):
    logger.info(
        'OCR complete for document file: %s', document_version_pk
    )
    ...
    try:
        ...
        logger.error(
            'Exception in OCR finish for document file: %d; %s', document_version_pk,
            exception
        )
        document_version.ocr_errors.create(result=exception)
        raise
